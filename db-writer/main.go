...
var (
	...
	// how often the writer should write on the database; maps to WRITE_INTERVAL env
...
func main() {
	log.Println("DB Writer is starting...")
	initVariables()
	var err error
	...
	// Instantiate and start the random writer
	rw := NewRandomWriter(db, randomSeed)
	errCh := make(chan error)
	go rw.Write(ctx, writeInterval, log.Writer(), errCh)
	...
	case <-exitToRebornCh:
		...
		os.Exit(36)
	case err = <-errCh:
		log.Println("error happend during write operations, we stop.")
		break
	}
